...
app.get("/random/", async (req, res)=>{
  try{
    ...
    const randomJoke = jokes[index].jokeText;
    const typeOfJoke = jokes[index].jokeType;
    res.json(
      {
        jokeText: randomJoke,
        jokeType: typeOfJoke
...
app.get("/jokes/:id", async (req, res)=>{
  try {
    ...
    const joke = jokes.find(j => j.id === searchId);
    res.json({
...
app.get("/filter", async (req, res)=>{
  try {
    ...
    const filteredJokes = jokes.filter(j=>j.jokeType === searchType);
...
app.put("/jokes/:id", async(req, res)=>{
  try {
      ...
      const index = jokes.findIndex(j=>j.id===searchId);
...
app.patch("/jokes/:id", async(req, res)=>{
  try {
      const searchId = parseInt(req.params.id);
      const existingJoke = jokes.find(j => j.id === searchId);
      const edittedJoke = {
        id: searchId,
      ...
      const index = jokes.findIndex(j => j.id === searchId);
      jokes[index] = edittedJoke;
      console.log(jokes[index]);
      res.json(edittedJoke);
  } catch (error) {
    console.log("Error", error.message);
    res.status(500).json({error: "Internal Server Error"});
  }
...
app.delete("/jokes/:id", async(req, res)=>{
  try{
    const searchId = parseInt(req.params.id);
    const indexOfJoke = jokes.findIndex(j=>j.id===searchId);
    if(indexOfJoke===-1){
      console.log("Data does not exist!");
      return res.status(404).json({error: "Joke not found!"});
    }
    jokes.splice(indexOfJoke, 1);
...
//8. DELETE All jokes
//Arrange the postman side (authorization -> API Key)
